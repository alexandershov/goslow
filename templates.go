...
curl -d "{{ .TruncatedBody }}" "{{ .AdminDomain }}{{ .AdminUrlPathPrefix }}{{ .Path }}?delay=2.5&method=GET"
...
curl -d "{{ .TruncatedBody }}" "{{ .CreateDomain }}{{ .AdminUrlPathPrefix }}{{ .Path }}?delay=2.5&method=GET"
... and you're done!
`))
